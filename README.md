...
- **Firebase Hosting**: `firebase deploy` from `dist/` directory
...
### Git Management
- The `dist/` directory is automatically gitignored
- Only source files in `web/` and `src/` are tracked
- Build artifacts are regenerated on each deployment
- Old build files are cleaned before each new build
